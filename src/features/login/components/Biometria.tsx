  ...
  return (
    <div className="relative flex size-28 items-center justify-center">
      {fase === 'ok' ? (
        ...
        <>
          <div className={`absolute inset-0 rounded-full border-2 border-accent/25 ${fase === 'escaneando' ? 'animate-[anillo_1.2s_ease-out_infinite]' : ''}`} />
          <div className={`absolute inset-3 rounded-full ${fase === 'escaneando' ? 'bg-accent-light' : 'bg-card-2'} transition`} />
          <Fingerprint size={56} strokeWidth={1.25} className={`relative text-ink-secondary ${fase === 'escaneando' ? 'opacity-100' : 'opacity-70'}`} />
          {fase === 'escaneando' && (
